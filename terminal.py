...
# --- 1. THE "MATRIX" LED TICKER (Full Colorful Cycle) ---
st.set_page_config(layout="wide", page_title="TERMINAL_V17_STABLE", initial_sidebar_state="collapsed")

# 30+ Assets for a high-density institutional feel
...
try:
    ...
    with c:
        ...
        with tabs[3]:
            ...
            st.write(f"*AS OF: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | FREQUENCY: 60 SECS*")
            st.write("---")
            if news:
                for item in news[:6]: # Top 6 real-time headlines
