...
# --- 1. THE TERMINAL SKIN (Fixed for 2026) ---
st.set_page_config(layout="wide", page_title="TERMINAL_v2.0_QUANT")

# Using st.html for the 'Matrix' feel without the TypeError
st.html("""
    <style>
...
st.title("📟 QUANT_EXEC // STABLE_v2.1")
...
# --- 3. DATA ENGINE (Fixed for Series formatting) ---
@st.cache_data
...
try:
    ...
    # FIX: We use .item() or float() to get the single last value, avoiding the Series error
    ...
    with col_stats:
        # Convert stats to a clean dataframe for display
