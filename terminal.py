...
import google.generativeai as genai  # Ensure 'google-generativeai' is in requirements.txt

# --- 1. MAJESTIC NEON UI OVERHAUL ---
st.set_page_config(layout="wide", page_title="SOVEREIGN_GEMINI_V30", initial_sidebar_state="collapsed")
...
ticker_html = "".join([f"<span style='color:{'#00ff41' if i%2==0 else '#00f0ff'}; padding-right:100px;'>{t}: LIVE_WIRE</span>" for i, t in enumerate(tickers)])
...
st.markdown(f"""
    ...
    .gazette-body {{ border: 10px double #00ff41; padding: 60px; background: #000; color: #00ff41; margin-top: 30px; box-shadow: 0 0 60px rgba(0,255,65,0.15); }}
    .gazette-title {{ font-family: 'Orbitron', sans-serif; font-size: 85px; font-weight: 900; border-bottom: 10px solid #00ff41; text-align: center; text-transform: uppercase; letter-spacing: -5px; line-height: 0.85; padding-bottom: 10px; }}
    .column-wrapper {{ column-count: 2; column-gap: 60px; text-align: justify; line-height: 1.8; font-size: 17px; }}
    .dropcap {{ float: left; font-size: 95px; line-height: 75px; padding-top: 5px; padding-right: 15px; font-weight: bold; color: #00ff41; }}
    </style>
    <div class="led-ticker"><div>{ticker_html} | {ticker_html}</div></div>
""", unsafe_allow_html=True)

# --- 2. PERSISTENT STATE & LOCKED LOGIC ---
...
def fetch_terminal_data(ticker):
    ...
    return df, s.info, s.quarterly_financials, s.options, s.news
...
try:
    ...
    with l:
        ...
        st.markdown("---")
        # API Key Input
        st.session_state.gemini_key = st.text_input("GEMINI_KEY", type="password", placeholder="Paste Gemini API Key...")
        st.markdown("---")
    ...
    with c:
        ...
        tabs = st.tabs(["📊 ANALYSIS", "📈 GEX_PROFILE", "📉 OPTIONS", "💰 FINANCIALS", "📰 THE_AI_GAZETTE"])
        ...
        with tabs[4]:
            # --- THE GEMINI GHOSTWRITER ENGINE ---
            if st.session_state.gemini_key:
                genai.configure(api_key=st.session_state.gemini_key)
                model = genai.GenerativeModel('gemini-1.5-flash')
                
                headlines = "\n".join([f"- {n['title']}" for n in live_news[:5]]) if live_news else "No news headlines available."
                prompt = (f"Act as a cynical, humorous institutional Wall Street writer for 'The Sovereign Gazette'. "
                          ...
                          f"Use snarky financial slang, mention 'retail bagholders', and don't use templates. "
                          f"Format it as a physical newspaper article.")
                ...
                ai_story = "THE GHOSTWRITER IS OFFLINE. Paste a Gemini API Key in the HUD to authorize sentient reporting."
            ...
            st.markdown(f"""
                <div class="gazette-body">
                    ...
                    <div class="gazette-sub">
                        ...
                        <span>SENTIMENT: GEMINI_GENERATED</span>
                    </div>
                    <div class="column-wrapper">
                        <p><span class="dropcap">{ai_story[0] if ai_story else 'T'}</span>{ai_story[1:] if ai_story else 'The news is silent.'}</p>
                    </div>
                </div>
            """, unsafe_allow_html=True)
