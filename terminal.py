...
st.set_page_config(layout="wide", page_title="SOVEREIGN_V37_LOCK", initial_sidebar_state="collapsed")
...
def get_marquee_html(tickers):
    ...
    for t in tickers:
        try:
            # Replaced "live_wire" with actual price feeds
...
st.markdown(f"""
    ...
    .gazette-body {{ border: 8px double #00ff41; padding: 60px; background: #000; color: #00ff41; margin-top: 30px; }}
    .gazette-title {{ font-family: 'Orbitron', sans-serif; font-size: 90px; text-align: center; text-transform: uppercase; border-bottom: 10px solid #00ff41; }}
    </style>
        ...
        with tabs[4]:
            ...
            if GEMINI_API_KEY:
                try:
                    ...
                    prompt = f"Snarky news report for {st.session_state.ticker}. Price: ${df['Close'].iloc[-1]}. Headlines: {headlines}."
                    response = model_engine.generate_content(prompt)
                    ai_story = response.text
                except Exception as e:
                    ai_story = f"SYSTEM_ERROR: {e}"
            
            st.markdown(f"<div class='gazette-body'><div class='gazette-title'>{st.session_state.ticker} Gazette</div><p>{ai_story}</p></div>", unsafe_allow_html=True)
