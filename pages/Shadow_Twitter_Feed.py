...
# This ensures the map and feed update automatically
st_autorefresh(interval=60000, key="global_refresh")

# --- 2. GHOST ENGINE SETUP ---
...
# --- 3. AUTOMATIC TANKER DISCOVERY ---
@st.cache_data(ttl=300) 
def scrape_live_ais():
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    # Targeted search for 'Laden Tankers'
    ...
    try:
        ...
        for row in rows[:8]: # Grab top 8
            ...
            if ship_link:
                ...
                # Convert to Float for PyDeck map
        ...
        return pd.DataFrame([{"name": "SCANNING...", "lat": 0.0, "lon": 0.0}])

# --- 4. SHADOW X FEED (COOKIE FORMATTER) ---
async def get_shadow_tweets(ticker):
    client = Client('en-US')
    try:
        with open('cookies.json', 'r') as f:
            raw = json.load(f)
        # Formatter: Extracts 'name' and 'value' from your list format
...
# Automatic Sidebar Data Fetch
ticker = st.sidebar.text_input("📡 TARGET TICKER", value="NVDA").upper()
tanker_df = scrape_live_ais()

t1, t2 = st.tabs(["🌍 LIVE_AIS_MAP", "𝕏 INTEL_STREAM"])

with t1:
    # High-Tech World Map (CARTO_DARK fixes the black void)
...
with t2:
    ...
    for post in feed:
        ...
        <div style="background:#000; border:1px solid #2f3336; padding:15px; border-radius:12px; margin-bottom:10px;">
            <div style="color:#fff; font-weight:bold;">𝕏 {post.get('User')} <span style="color:#71767b;">@{post.get('Handle', 'anon')}</span></div>
